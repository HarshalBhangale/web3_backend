...
# Stop loss trigger
# Make profit trigger
# Get info about nfts, stocks etc in anykind of filter way
# Buy sell nfts
# Show portfolio of crypto and nfts etc
...
def parse_unstructured_text(input):
    prompt = f"""
            Assume you are a human being, who parses english text into a json response.
    ...
    try:
        obj = response["choices"][0]["text"].replace('Response:', '').strip()
        j = json.loads(obj)
